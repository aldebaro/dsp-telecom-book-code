...
try:
    while True:
        r = sd.playrec(
            np.zeros(recordingDuration * Fs), Fs, channels=2
        )  # recordingDuration*Fs = number of samples
        sd.wait()  # Wait until recording is finished
        sd.sleep(
            int(recordingDuration * 1000)
        )  # Wait for the recordDuration in milliseconds
        print(r)
