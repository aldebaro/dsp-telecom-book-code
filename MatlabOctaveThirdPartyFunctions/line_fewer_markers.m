...
if  isvector(x) &&  isvector(y)
    ...
    x=transpose(x(:));
    y=transpose(y(:));
    %if iscolumn(x), x = x.'; end  %AK: iscolumn not supported in my version
...
%AK
%if isempty(Spacing),    Spacing     = 'curve'   ; end %%'x' -> marker delta-x constant; 'curve' : spacing constant along the curve length
