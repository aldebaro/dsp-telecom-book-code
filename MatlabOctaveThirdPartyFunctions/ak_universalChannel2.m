%Example of a LTI Gaussian channel: filter + noise addition
...
noiseSamples = zeros(size(playSampleBuffer); %initialize noise array
while true %script runs until interrupted (by CTRL + C, for example)
    ...
    if (~isempty(recSamples)) %DSP code section
        ...
        noiseSamples = randn(size(noiseSamples)); %new noise samples
