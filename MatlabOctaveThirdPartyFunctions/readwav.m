...
if ~info(3)
   ...
   if ~isequal(header',uint8('WAVE')) fclose(fid); error(sprintf('File does not begin with a WAVE chunck')); end
   ...
   while ~data						% loop until FMT and DATA chuncks both found
      ...
      %char(header')
      if isequal(header',uint8('fmt '))					% ******* found FMT chunk *********
         fmt=1;
      ...
      elseif isequal(header',uint8('data'))				% ******* found DATA chunk *********
