function [Y, Rhh] = ak_mafi(r,L,T_SEQ,OSR)
%
% AK_MAFI: Estimates the channel impulse, performs bit
% syncronization, matched filtering and downconversion
% to the baud rate.
%
% SYNTAX:   [y, Rhh] = ak_mafi(r,Lh,T_SEQ,OSR)
%
...
r_sub = r(start_sub:end_sub); %do not use the whole burst
...
%the received burst
%Pre-allocate space:
crossCorrelation = zeros(1,length(r_sub)-OSR*16);
...
% USED PART OF TRAINING SEQUENCE, WHICH IS 3+57+1+6=67
% BITS INTO THE BURST. THAT IS WE HAVE THAT sync_T16
% EQUALS FIRST SAMPLE IN BIT NUMBER 67.
