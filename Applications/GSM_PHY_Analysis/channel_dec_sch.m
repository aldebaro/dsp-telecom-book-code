function [rx_block,FLAG_SS,PARITY_CHK] = channel_dec_sch(rx_enc)
%
% channel_dec: 
%
% SYNTAX:     [rx_block, FLAG_SS, PARITY_CHK] = channel_dec(rx_enc)
% 
% INPUT:      rx_enc  A 78 bits long vector contaning the encoded 
%                     data sequence as estimated by the SOVA. The 
%                     format of the sequency must be according to 
%                     the GSM 05.03 encoding scheme
%
% OUTPUT:     rx_block  A 39 bits long vector contaning the final
...
% HENCE ONLY 78/2 = 39 STATE TRANSITIONS OCCUR
%
...
%
% THE TABLE IS SETUP SO THAT THE CALL DIBIT(X,Y) RETURNS
...
% THE TABLE IS SETUP SO THAT THE CALL BIT(X,Y) RETURNS
