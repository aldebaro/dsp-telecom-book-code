...
%function [ann_handle,figx,figy,orix,oriy]=ak_drawvector(x1,y1,xo,yo)
%Draws an arrow. Examples (all are equivalent for drawing a
%vector from point (0,0) to (2,1):
%ak_drawvector(2+j)
%ak_drawvector(2,1)
%ak_drawvector([2,1])
%ak_drawvector([2,1]')
...
%eventually expand axis to avoid error of out of bounds when drawing
currentAxis=axis; %assume it is a 2-d graph
xmin = min([currentAxis(1) x xo]);
xmax = max([currentAxis(2) x xo]);
ymin = min([currentAxis(3) y yo]);
ymax = max([currentAxis(4) y yo]);
...
% dsxy2figxy -- Transform point or position from axis to figure
% coordinates.
% Transforms [axx axy] or [xypos] from axes hAx (data) coords into
% coords
% wrt GCF for placing annotation objects that use figure coords into
% data space. The annotation objects this can be used for are
