...
Xmag=20*log10(abs(X)/Nfft);
h=plot(f,Xmag);
maxIndex=find(Xmag==max(Xmag),1,'last');
maxFFTValue=X(maxIndex); %return value
maxFreq=f(maxIndex); %return value
